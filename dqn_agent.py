...
class DQN_class:
    ...
    replay_start_size = 100         # No. of uniform random policy is run before learning start 
                                    # original: 5x10^4
    replay_mem_size = 10**5         # Data size of history for SGD update. original: 10^6
    batch_size = 32                 # Replay (batch) size. original: 32
    target_nn_update_freq = 10**4   # Target update frequancy. original:10^4

    def __init__(self, enable_controller=[1, 2]):
        ...
        self.D = [np.zeros((self.replay_mem_size, 1, 4), dtype=np.uint8),
                  np.zeros(self.replay_mem_size, dtype=np.uint8),
                  np.zeros((self.replay_mem_size, 1), dtype=np.int8),
                  np.zeros((self.replay_mem_size, 1, 4), dtype=np.uint8),
                  np.zeros((self.replay_mem_size, 1), dtype=np.bool)]
    ...
    def forward(self, state, action, Reward, state_dash, episode_end):
        ...
        zero_val = Variable(np.zeros((self.batch_size, self.num_of_actions), dtype=np.float32))
    ...
    def stockExperience(self, time,
                        ...
                        episode_end_flag):
        data_index = time % self.replay_mem_size
    ...
    def experienceReplay(self, time):
        ...
        if self.replay_start_size < time:
            # Pick up batch_size number of samples from the Data
            if time < self.replay_mem_size:  # during the first sweep of the History Data
                replay_index = np.random.randint(0, time, (self.batch_size, 1))
            else:
                replay_index = np.random.randint(0, self.replay_mem_size, (self.batch_size, 1))
            ...
            s_dash_replay = np.ndarray(shape=(self.batch_size, 4, 84, 84), dtype=np.float32)
            episode_end_replay = np.ndarray(shape=(self.batch_size, 1), dtype=np.bool)
...
class dqn_agent(Agent):  # RL-glue Process
    ...
    def agent_step(self, reward, observation):
        ...
        if self.policyFrozen is False:  # Learning ON/OFF
            if self.DQN.replay_start_size < self.time:
                self.epsilon -= 1.0/10**6
                if self.epsilon < 0.1:
                    self.epsilon = 0.1
                eps = self.epsilon
            else:  # Initial Exploration Phase
                print "Initial Exploration : %d/%d steps" % (self.time, self.DQN.replay_start_size)
                eps = 1.0
        else:  # Evaluation
        ...
        # Target model update
        if self.DQN.replay_start_size < self.time and np.mod(self.time, self.DQN.target_nn_update_freq) == 0:
            print "########### MODEL UPDATED ######################"
